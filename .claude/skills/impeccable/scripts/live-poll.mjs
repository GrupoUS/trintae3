 ...
 *
 * Usage:
 *   bunx impeccable poll                         # Block until browser event, print JSON
 *   bunx impeccable poll --timeout=600000        # Custom timeout (ms); default is long-poll friendly
 *   bunx impeccable poll --reply <id> done       # Reply "done" to event <id>
 *   bunx impeccable poll --reply <id> error "msg" # Reply with error
 */
...
function readServerInfo() {
  ...
  if (!record) {
    console.error(
      "No running live server found. Start one with: bunx impeccable live",
    );
    process.exit(1);
  }
  return record.info;
...
Options:
  ...
  // Reply mode: bunx impeccable poll --reply <id> <status> [--file path] [message]
  ...
  if (replyIdx !== -1) {
    ...
    if (!id) {
      console.error(
        "Usage: bunx impeccable poll --reply <id> <status> [--file path] [message]",
      );
      process.exit(1);
    }
    ...
    try {
      ...
      if (err.cause?.code === "ECONNREFUSED") {
        console.error(
          "Live server not running. Start one with: bunx impeccable live",
        );
      } else {
  ...
  try {
    while (true) {
      ...
      if (res.status === 401) {
        ...
        console.error(
          "Try restarting: bunx impeccable live stop && bunx impeccable live",
        );
        process.exit(1);
      }
